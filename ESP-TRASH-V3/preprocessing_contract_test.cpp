...
int main() {
  ...
  if (preprocessor.Configure(1.0F / 255.0F, -128) != aiot::Status::kOk) {
    std::cerr << "V9 preprocessor configure failed\n";
    return 1;
  }
  ...
  if (preprocessor.Run(view, output.data(), output.size()) != aiot::Status::kOk) {
    std::cerr << "V9 preprocessor run failed\n";
    return 1;
  }
  ...
  if (hash != kExpectedFnv1a) {
    std::cerr << "V9 preprocessing mismatch: 0x" << std::hex << hash
              << " != 0x" << kExpectedFnv1a << '\n';
    return 1;
  }
  std::cout << "V9 preprocessing contract: PASS\n";
  return 0;
}
