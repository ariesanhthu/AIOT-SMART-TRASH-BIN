"""Verify that ESP-TRASH-V4 embeds the exact current AI/V10 INT8 model."""
...
ARTIFACTS = REPOSITORY_DIR / "AI" / "V10" / "artifacts"
EXPECTED_LABELS = ["paper", "plastic", "organic"]
EXPECTED_MODEL_VERSION = "tinycnn-v10-wide-128x96-esp-contract"
...
def main() -> None:
    ...
    if metadata["int8_model"]["bytes"] != len(model):
        raise RuntimeError("V10 metadata model size does not match the TFLite file")
    if metadata["int8_model"]["sha256"] != expected_hash:
        raise RuntimeError("V10 metadata model hash does not match the TFLite file")
    if quantization["sha256"] != expected_hash or deployment["sha256"] != expected_hash:
        raise RuntimeError("V10 quantization/deployment model hash differs")
    if metadata.get("model_version") != EXPECTED_MODEL_VERSION:
        raise RuntimeError("V10 model version is invalid")
    if metadata.get("labels") != EXPECTED_LABELS or deployment["labels"] != EXPECTED_LABELS:
        raise RuntimeError("V10 label order is invalid")
    if quantization["input"]["shape"] != [1, 96, 128, 3]:
        raise RuntimeError("V10 input shape is invalid")
    if quantization["output"]["shape"] != [1, len(EXPECTED_LABELS)]:
        raise RuntimeError("V10 output shape is invalid")
    if quantization["float_tensors"]:
        raise RuntimeError("V10 deployment model contains floating tensors")
    ...
    if embedded != model:
        raise RuntimeError("Embedded byte array differs from AI/V10/model_int8.tflite")
    ...
    if int(_capture(contract, r"kExpectedModelBytes\s*=\s*(\d+)\s*;")) != len(model):
        raise RuntimeError("model_contract.h model length differs from V10")
    if _capture(
        contract, r'kExpectedModelSha256\[\]\s*=\s*\n?\s*"([0-9a-f]{64})"\s*;'
    ) != expected_hash:
        raise RuntimeError("model_contract.h model hash differs from V10")
