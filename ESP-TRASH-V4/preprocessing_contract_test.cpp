...
constexpr std::size_t kWidth = 211U;
constexpr std::size_t kHeight = 123U;
constexpr std::size_t kChannels = 3U;
constexpr std::uint64_t kExpectedFnv1a = 0x3f77e533a972a47fULL;
...
int main() {
  ...
  if (preprocessor.Configure(1.0F / 255.0F, -128) != aiot::Status::kOk) {
    std::cerr << "V10 preprocessor configure failed\n";
    return 1;
  }
  ...
  std::vector<std::int8_t> output(
      static_cast<std::size_t>(aiot::model_contract::kInputWidth) *
      static_cast<std::size_t>(aiot::model_contract::kInputHeight) * kChannels);
  if (preprocessor.Run(view, output.data(), output.size()) != aiot::Status::kOk) {
    std::cerr << "V10 preprocessor run failed\n";
    return 1;
  }
  ...
  if (hash != kExpectedFnv1a) {
    std::cerr << "V10 preprocessing mismatch: 0x" << std::hex << hash
              << " != 0x" << kExpectedFnv1a << '\n';
    return 1;
  }
  std::cout << "V10 preprocessing contract: PASS\n";
  return 0;
}
