"""Train TinyCNN V4 with a balanced reject class and grouped V3 splits."""
...
from V4.runtime import LABELS, MODEL_VERSION, configure_shared_contract
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--paper-weight-multiplier", type=float, default=1.15)
    parser.add_argument("--plastic-weight-multiplier", type=float, default=1.15)
...
def main() -> None:
    ...
    class_weights[LABELS.index("paper")] *= args.paper_weight_multiplier
    class_weights[LABELS.index("plastic")] *= args.plastic_weight_multiplier
    ...
    deployment_model = build_tiny_cnn_v4()
    ...
    test_metrics = classification_metrics(
        test_truth, _predict(deployment_model, test_dataset)
    )
    ...
    config = {
        **vars(args),
        ...
        "offline_augmentation": "V3 preserved plus 25% ESP-simulated other train",
    ...
    metadata["training_variant"] = (
        "AI/V4 V3-compatible TinyCNN with balanced cardboard/metal reject class"
    )
...
class ValidationSelectionMetrics(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch: int, logs: dict | None = None) -> None:
        ...
        print(
            f" - val_macro_f1: {macro_f1:.4f} - val_min_recall: {min_recall:.4f} "
...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    if args.paper_weight_multiplier <= 0 or args.plastic_weight_multiplier <= 0:
