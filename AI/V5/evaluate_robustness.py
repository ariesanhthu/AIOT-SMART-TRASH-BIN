"""Evaluate V5 and V4 on clean and deterministic environmental stress profiles."""
...
DEFAULT_ARTIFACTS = V5_DIR / "artifacts"
PROFILE_NAMES = (
    "clean",
    "low_light",
    "overexposed",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min-clean-macro-f1", type=float, default=0.80)
    parser.add_argument("--min-stress-macro-f1", type=float, default=0.60)
    parser.add_argument("--max-organic-to-paper", type=float, default=0.20)
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    for name in ("min_clean_macro_f1", "min_stress_macro_f1", "max_organic_to_paper"):
    ...
    models: dict[str, object] = {
        "v5_float": float_predictor,
        "v5_int8": int8_predictor,
    }
    ...
    for model_name, predictor in models.items():
        ...
        for profile, images in profile_images.items():
    ...
    clean_min_recall = min(
        float(v5["clean"]["per_class"][label]["recall"]) for label in LABELS
    )
    gates = {
        "clean_macro_f1_passed": v5["clean"]["macro_f1"] >= args.min_clean_macro_f1,
        "clean_min_recall_passed": clean_min_recall >= 0.75,
        "mean_stress_macro_f1_passed": (
            v5["summary"]["mean_stress_macro_f1"] >= args.min_stress_macro_f1
        ),
        "clean_organic_to_paper_passed": (
            v5["clean"]["organic_to_paper_rate"] <= args.max_organic_to_paper
        ),
        "stress_organic_to_paper_passed": (
            v5["summary"]["mean_stress_organic_to_paper_rate"]
...
def _write_profile_chart(path: Path, results: dict[str, dict]) -> None:
    ...
    axis.set_title("Clean and environmental robustness (untouched V5 test split)")
