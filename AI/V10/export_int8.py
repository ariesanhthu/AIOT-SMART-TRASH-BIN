"""Quantize V10 to a full-INT8, ESP32-compatible TFLite model."""
...
def main() -> None:
    ...
    if not model_path.is_file():
        raise FileNotFoundError(f"V10 float model not found: {model_path}")
    ...
    def representative_dataset():
        ...
        yield [np.zeros((1, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS), np.float32)]
        yield [np.ones((1, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS), np.float32)]
...
def inspect_and_evaluate(path: Path, samples) -> dict:
    ...
    flat = (
        np.arange(IMAGE_HEIGHT * IMAGE_WIDTH * IMAGE_CHANNELS, dtype=np.uint32) * 191 + 7
    ) & 255
