"""Train V8 from AI/V8/dataset_prepared."""
...
from V8.model import build_model
...
def main() -> None:
    ...
    steps = steps_per_epoch(
        train_samples, args.batch_size, args.views_per_source
    )

    validation_truth = np.asarray([s.label_id for s in validation_samples])
    ...
    history = model.fit(
        ...
        callbacks=[
            ...
            tf.keras.callbacks.ModelCheckpoint(
                checkpoint, monitor="val_selection_score", mode="max",
            ...
            tf.keras.callbacks.ReduceLROnPlateau(
                monitor="val_selection_score", mode="max", factor=0.5,
                patience=max(4, args.patience // 2), min_lr=1e-5, verbose=1,
            ),
    ...
    metadata = {
        ...
        "augmentation": {
            **AUGMENTATION_CONFIG,
            "views_per_source_per_epoch": args.views_per_source,
    ...
    print(json.dumps({
        "model": str(model_path), "best_epoch": best_epoch,
...
class ValidationMetrics(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch: int, logs: dict | None = None) -> None:
        ...
        score = 0.65 * metrics["macro_recall"] + 0.35 * metrics["accuracy"] - 0.001 * loss
...
def _evaluate(model, dataset, samples) -> dict:
    truth = np.asarray([s.label_id for s in samples], dtype=np.int64)
    return classification_metrics(truth, model.predict(dataset, verbose=0))
...
def _validate_args(args) -> None:
    if args.epochs < 1 or args.batch_size < 1 or args.views_per_source < 1:
        raise ValueError("epochs, batch-size and views-per-source must be positive")
