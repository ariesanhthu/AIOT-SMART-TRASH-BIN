"""Manifest-driven V9 input pipeline with one shared ESP32 preprocessing path."""
...
def load_samples(root: str | Path = DATASET_DIR) -> tuple[Sample, ...]:
    ...
    if not manifest.is_file():
        raise FileNotFoundError(f"V9 manifest does not exist: {manifest}")
    ...
    for row in rows:
        ...
        if digest != row["sha256"]:
            raise ValueError(f"V9 checksum mismatch: {path}")
        if digest in hashes:
        ...
        if online_augment:
            raise ValueError(
                f"V9 requires saved augmentation files; online augmentation is forbidden: {path}"
            )
...
def center_crop_resize_u8(image: tf.Tensor) -> tf.Tensor:
    """Center-square crop and nearest-neighbor floor resize used by firmware."""
