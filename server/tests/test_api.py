...
def test_frontend_compatible_endpoints(tmp_path: Path) -> None:
    ...
    config = client.patch(
        ...
        json={"thresholds": {"plastic": 90}, "maintenanceMode": True},
    )
    ...
    loaded_device = client.get("/api/devices/esp32cam-01").json()
    assert loaded_device["compartments"]["plastic"]["threshold"] == 0.9

    restarted_client = TestClient(
        create_app(data_dir=tmp_path, device_token="test-token")
    )
    persisted_config = restarted_client.get(
        "/api/devices/esp32cam-01/config"
    ).json()
    assert persisted_config["thresholds"]["plastic"] == 0.9


def test_devices_endpoint_returns_latest_fill_levels_without_cache(
    tmp_path: Path,
) -> None:
    app = create_app(data_dir=tmp_path, device_token="test-token")
    client = TestClient(app)
    ...
    latest_headers = detection_headers()
    latest_headers.update(
        {
            "X-Fill-Plastic": "11",
            "X-Fill-Paper": "82",
            "X-Fill-Organic": "17",
    ...
    devices = client.get("/api/devices")
    assert devices.status_code == 200
    assert devices.headers["cache-control"] == "no-store, no-cache, must-revalidate"
    compartments = devices.json()[0]["compartments"]
    assert compartments["plastic"]["fillPercent"] == 11
    assert compartments["paper"]["fillPercent"] == 82
    assert compartments["organic"]["fillPercent"] == 17
